...
function renderProduct(product) {
  return `<div class="grid g1 pt2 border-top border-fuchsia" style="grid-template-rows: repeat(2, 1fr); grid-template-columns: min-content auto;" >
            <img src="./products/${product.id}.avif" style="width: 100px; grid-row: 1 / -1; grid-column: 1 / 2;" />
            <div class="flex justify-between">
                ${product.title}
                <span>${priceFormatter.format(product.price)}</span>
            </div>
            <img src="./icons/add-to-cart.svg" class="border-none ml-auto cursor-pointer" onclick="updateCart('${product.id}', 1)" />
        </div>`;
}
...
function updateCart(productId, difference) {
  ...
  document.getElementById("cart-count").textContent = products;
...
function renderCartProduct(product, count) {
  return `
    <div class="grid g1 py1" style="grid-template-rows: auto auto; grid-template-columns: auto max-content;">
        <div style="grid-column: 1; grid-row: 1 / -1">${product.title}</div>
        <span class="right-align" style="grid-column: 2; grid-row: 1">${count} × ${
    priceFormatter.format(product.price)
  }</span>
        <a class="font-small right-align" style="grid-column: 2; grid-row: 2" onclick="updateCart('${product.id}', ${-count})">Remove</a>
    </div>
    `;
}
