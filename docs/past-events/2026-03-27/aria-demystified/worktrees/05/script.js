...
function renderSearching() {
  ...
  main.ariaDescribedBy = "loading-spinner";
  ...
  productsContainer.innerHTML =
    '<div id="loading-spinner" role="progressbar" aria-label="Loading" class="spinner mx-auto"></div>';
}
...
function renderProducts(products, filtered) {
  const main = document.querySelector("main");
  main.ariaDescribedBy = undefined;
...
function renderProduct(product) {
  ...
  return `<li class="grid g1 pt2 border-top border-fuchsia" style="grid-template-rows: repeat(2, 1fr); grid-template-columns: min-content auto;" >
            ...
            <button id="${cartId}" aria-label="Add to cart" aria-labelledby="${cartId} ${titleId}" class="ghost ml-auto" onclick="updateCart('${product.id}', 1)" >
              <img alt="" src="./icons/add-to-cart.svg" />
            </button>
        </li>`;
}
...
function renderCartProduct(product, count) {
  ...
  return `
    <li class="grid g1 py1" style="grid-template-rows: auto auto; grid-template-columns: auto max-content;">
    ...
    priceFormatter.format(product.price)
        ...
        <button
          id="${removeId}"
          aria-labelledby="${removeId} ${titleId}
          aria-label="Remove
          class="ghost fuchsia ml-auto font-small right-align
          style="grid-column: 2; grid-row: 2
          onclick="updateCart('${product.id}', ${-count})">
            Remove
        </a>
    </li>
    `;
